...
func run() -> void:
	...
	DirAccess.make_dir_recursive_absolute("res://assets/textures/skyboxes")
	...
	check(result.game_path == ProjectSettings.globalize_path("res://").trim_suffix("/"), "Game Path is project root")
	check(result.message.contains(result.game_path), "Status shows project root")
	check(config.filesystem.searchpath == ".", "Filesystem uses project root")
	var texture_directory: String = result.game_path.path_join(config.materials.root)
	check(config.materials.root == ".godot/trenchbroom/assets/textures", "Texture search is scoped to generated catalog")
	check(not FileAccess.file_exists(texture_directory.path_join("skyboxes/sky.png")), "Unselected skybox excluded")
	check(not FileAccess.file_exists(result.game_path.path_join("assets").path_join(config.materials.root).path_join("skyboxes/sky.png")), "Assets mod cannot expose unselected skybox through catalog root")
	...
	check(FileAccess.file_exists(texture_directory.path_join("metal/red.png")), "Failed export preserves asset catalog")
	...
	for version in [4, 8, 9]:
		...
		check(config[texture_key].palette == ".godot/trenchbroom/assets/palette.lmp", "Palette relative to project root")
		check(FileAccess.file_exists(result.game_path.path_join(config[texture_key].palette)), "Palette resolves from Game Path")
		var material_root: String = config[texture_key].package.root if version == 4 else config[texture_key].root
		check(FileAccess.file_exists(result.game_path.path_join(material_root).path_join("stone/red.png")), "Textures resolve from Game Path in each schema")
	...
	check(not FileAccess.file_exists(texture_directory.path_join("metal/red.png")), "Re-export removes stale copied textures")
